...
import {
  ...
  UNLINK_PROJECT_CONSTRAINT_FROM_PROJECT_CHARTER,
} from '../api/constraintApi'

/**
 * Mutation hook for removing the link between a project constraint and a project charter.
 *
 ...
 */
export function useUnlinkProjectConstraintFromProjectCharter(scopeType: string, scopeId: string) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      await graphqlClient.request(UNLINK_PROJECT_CONSTRAINT_FROM_PROJECT_CHARTER, {
        constraintId,
        projectCharterId,
      })
    },
