...
/** Allowed scope types for a project initiation request. */
export const scopeTypeValues = ['Program', 'Portfolio'] as const
/** Union type derived from {@link scopeTypeValues}. */
...
/**
 * Draft-save schema: only `name`, `requestingProjectId`, `scopeType`, and `scopeId` are required.
 * All other fields are optional per D-008 / data-model.md.
 */
...
export const submitSchema = draftSchema.extend({
  ...
  requestDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'invalidDate'),
...
export type ProjectInitiationRequestFormValues = z.infer<typeof draftSchema>
