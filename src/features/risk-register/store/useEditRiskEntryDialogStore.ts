...
/**
 * Zustand state shape for the Edit Risk Entry dialog.
 *
 ...
 */
interface EditRiskEntryDialogState {
  isOpen: boolean
  entryId: string | null
  open: (id: string) => void
  close: () => void
...
/**
 * Global store for the Edit Risk Entry dialog.
 *
 * Call `open(id)` to show the dialog for a specific entry and `close()` to hide it.
 */
export const useEditRiskEntryDialogStore = create<EditRiskEntryDialogState>((set) => ({
  isOpen: false,
  entryId: null,
