...
vi.mock('@/features/planning-objects', async (importOriginal) => {
  ...
  return {
    ...original,
    RequirementManagement: () => createElement('div', { 'data-testid': 'requirement-management' }),
  }
})
...
function renderPage(id = 'proj-1') {
  ...
  return render(
    createElement(
      MemoryRouter,
      { initialEntries: [`/projects/${id}/requirements`] },
      createElement(
        QueryClientProvider,
        { client: queryClient },
        createElement(
          ...
          createElement(Route, {
            ...
            element: createElement(ProjectRequirementsPage),
          }),
        ),
      ),
    ),
...
describe('ProjectRequirementsPage', () => {
  ...
  it('renders RequirementManagement', () => {
    renderPage()
    expect(screen.getByTestId('requirement-management')).toBeInTheDocument()
  })
  ...
  it('breadcrumb shows project name when loaded', async () => {
    const { useGetProject } = await import('@/entities/project')
    vi.mocked(useGetProject).mockReturnValue({ data: { name: 'Gamma Project' } } as ReturnType<
      typeof useGetProject
    >)

    renderPage('proj-abc')
    expect(screen.getByRole('link', { name: 'Gamma Project' })).toBeInTheDocument()
  })
